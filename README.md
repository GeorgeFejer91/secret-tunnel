...
Secret Tunnel launches the local MCP server, scopes it to the folder selected in the desktop app, and publishes that MCP endpoint through zrok. GitHub-related capabilities are exposed by the bundled MCP when the v2 GitHub bridge is present and the app is running in the corresponding access mode.
