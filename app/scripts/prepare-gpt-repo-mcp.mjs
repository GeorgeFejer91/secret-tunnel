...
const distNetwork = join(sourceDir, "dist", "network-service.js");
...
const networkHash = await sha256(distNetwork);
const lockHash = await sha256(packageLock);
const currentMarker = await readJson(markerPath);

if (
  currentMarker?.serverHash === serverHash &&
  currentMarker?.networkHash === networkHash &&
  existsSync(join(outputDir, "dist", "network-service.js")) &&
...
await copyFile(distNetwork, join(outputDir, "dist", "network-service.js"));
