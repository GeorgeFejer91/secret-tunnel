...
// The archive is passed as a bare filename with cwd set, deliberately. GNU tar
// reads an -f argument containing a colon as a remote "host:path" spec, so an
// absolute Windows path makes it try to reach a host called "C" and fail with
// "Cannot connect to C: resolve failed". Windows' own bsdtar has no such
// problem, which is why CI never saw this: only a developer whose PATH finds
// GNU tar first - anyone working in Git Bash or MSYS - hits it. A bare filename
// avoids the colon entirely and works with both implementations, whereas
// --force-local would fix GNU tar and be rejected by bsdtar. Only -f is parsed
// this way, so -C can stay absolute.
const tar = spawnSync("tar", ["-xzf", basename(archivePath), "-C", extractDir], {
