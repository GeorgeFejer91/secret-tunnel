...
export function wireNetwork(fit: () => void): void {
  ...
  let code = ''; let expiry = 0; let busy = false;
  ...
  async function refresh() {
    ...
    try {
      ...
      if (expiry && Date.now() >= expiry) { outgoing.value = ''; code = ''; expiry = 0; }
  ...
  element<HTMLButtonElement>('network-create').addEventListener('click', () => { void act(async () => {
    ...
    // The box shows the form that was asked for; the other two still work, and
    // Copy link and Copy code still reach them.
    outgoing.value = wantShort && result.short ? result.short : result.link;
    code = result.code; expiry = result.expiresAt;
    note(wantShort
  ...
  element<HTMLButtonElement>('network-copy-code').addEventListener('click', () => { void act(async () => { if (!code) throw new Error('Create an invitation first.'); await writeText(code); note('Invitation code copied.'); }); });
