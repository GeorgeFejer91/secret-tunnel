...
import { wireNetwork } from "./network";
import { wireSystemPrompt } from "./system-prompt";
import { wireStorage } from "./storage";
import { wireSmartFolders } from "./smart-folders";
...
const elements = {
  ...
  addFolder: mustElement<HTMLButtonElement>("add-folder"),
  folderList: mustElement<HTMLUListElement>("folder-list"),
  windowMinimize: mustElement<HTMLButtonElement>("window-minimize"),
  windowClose: mustElement<HTMLButtonElement>("window-close"),
...
let busy = false;
let smartFolders: ReturnType<typeof wireSmartFolders> | null = null;

window.addEventListener("DOMContentLoaded", () => {
  wireNetwork(fitAllText);
  wireSystemPrompt(fitAllText);
  wireStorage(fitAllText);
  smartFolders = wireSmartFolders(fitAllText);
  wireCabinetTabs();
  wireEvents();
  refresh();
  void refreshGitHubAccount();
  window.setInterval(refresh, 4000);
  // The four-second poll is throttled hard, sometimes to a stop, while the
  // window is hidden or behind another one - so the first thing a person sees
  // on coming back can be a frame from minutes ago. That is how a working
  // connection came to be reported as "Not running": the status was stale, not
  // wrong. Whenever the window is looked at again, read the real state first.
  for (const event of ["focus", "pageshow"]) {
    window.addEventListener(event, () => void refresh());
  }
...
/// The cabinet rail down the right edge. One pocket is open at a time: its
/// panel is in the layout and its tab is the one painted as part of it.
///
/// `hidden` rather than a CSS class, so a closed panel is out of the document
/// entirely - nothing inside it is focusable, and nothing inside it is read
/// out. The rail is one stop in the tab order, with the arrow keys moving
/// between pockets, which is what a tablist is expected to do.
function wireCabinetTabs() {
  const tabs = Array.from(
    document.querySelectorAll<HTMLButtonElement>(".cabinet-tab"),
  );
  if (tabs.length === 0) return;

  const open = (index: number) => {
    // The body takes the open sheet's paper; the stylesheet turns the index
    // into a colour, and transitions it.
    document.body.style.setProperty("--page-index", String(index));
    tabs.forEach((tab, at) => {
    ...
    // A hidden panel has no box to measure, so anything in it is fitted the
    // moment it has one.
    fitAllText();
  };

  // One full turn of the hue wheel spread across however many sheets there are,
  // so adding or removing a slot re-divides the deck instead of leaving a seam
  // where the colour jumps.
  document.body.style.setProperty("--hue-step", String(360 / tabs.length));

  tabs.forEach((tab, index) => {
    // Each sheet's own place in the deck, which is all the colour needs.
    tab.style.setProperty("--tab-index", String(index));
    tab.addEventListener("click", () => open(index));
...
/// The folder list, rebuilt from the status poll rather than kept in the DOM.
///
/// The backend is the only thing that knows which folders survived validation,
/// so a list edited in place here would drift from the folders actually served.
function renderFolderList(status: Status) {
  const primary = status.settings.workspacePath;
  const rows: Array<{ path: string; removable: boolean }> = [];
  if (primary) rows.push({ path: primary, removable: false });
  for (const folder of status.settings.extraFolders ?? []) {
    rows.push({ path: folder, removable: true });
  }

  elements.folderList.replaceChildren(
    ...rows.map(({ path, removable }) => {
      ...
      name.dataset.fit = "";
      // Split so the folder's own name stays readable when the path is long.
      ...
      if (removable) {
        ...
        remove.addEventListener("click", () =>
          runStatusCommand("remove_extra_folder", { path }),
        );
        item.append(remove);
      } else {
        const tag = document.createElement("span");
        tag.className = "folder-tag";
        tag.textContent = "GitHub folder";
        item.append(tag);
      }

      if (status.settings.smartScope) {
        // Under a smart scope a project is registered but not itself served,
        // so the row says so rather than reading as full access.
        const scoped = document.createElement("span");
        scoped.className = "folder-tag";
        scoped.textContent = "Scoped";
        scoped.title = `Only approved ${status.settings.smartScope.name} folders inside this project are exposed.`;
        item.append(scoped);
      }
      return item;
    }),
...
function wireEvents() {
  elements.addFolder.addEventListener("click", () => void addFolder());
  elements.windowMinimize.addEventListener("click", () => void getCurrentWindow().minimize());
  elements.windowClose.addEventListener("click", () => void getCurrentWindow().close());
...
async function addFolder() {
  await withBusy(async () => {
    // Null means the picker was dismissed, which is not a change to render.
    const status = await invoke<Status | null>("add_extra_folder");
    if (status) render(status);
  });
}
...
function render(status: Status) {
  ...
  renderFolderList(status);
  smartFolders?.render(status);
...
/// What each readiness gate is, in words a person can act on.
const STAGE_NAMES: Record<string, string> = {
  localProcess: "the local server process",
  localEndpoint: "the local address",
  localProtocol: "the local MCP handshake",
  publicTunnel: "the public tunnel",
  publicProtocol: "the MCP handshake through the tunnel",
};

/// The first gate that is not through yet, in the order a start actually
/// progresses: process, local address, local handshake, tunnel, handshake
/// through the tunnel.
///
/// Naming it is the whole difference between "not running" and "the share is
/// registered but the tunnel is not forwarding to this port" - the second is
/// something a person can do something about.
function firstStalledStage(readiness: Readiness): string | null {
  const stages = [
    readiness.localProcess,
    readiness.localEndpoint,
    readiness.localProtocol,
    readiness.publicTunnel,
    readiness.publicProtocol,
  ];
  for (const stage of stages) {
    if (stage.status === "verified" || stage.status === "unsupported") continue;
    const name = STAGE_NAMES[stage.kind] ?? stage.kind;
    return `${name}: ${stage.detail ?? `still ${stage.status}`}`;
  }
  return null;
}
...
function reachabilityIndicator(status: Status): Indicator {
  ...
  if (!status.running) {
    // "Not running" on its own says nothing about which part gave up, and the
    // gates below already know. Without this the same three words covered a
    // missing process, a tunnel forwarding to a closed port and a handshake
    // the local server refused.
    const stalled = readiness ? firstStalledStage(readiness) : null;
    return stalled === null
      ? { level: "bad", text: "Not running" }
      : { level: "bad", text: "Not running", reason: `Stopped at ${stalled}` };
  }
  ...
  if (publicProtocol.status === "failed" || publicTunnel.status === "failed") {
    const detail = firstStalledStage(readiness) ?? "the public URL is not responding";
    return { level: "bad", text: "Unreachable", reason: `ChatGPT cannot reach this app: ${detail}` };
  }
  // Still settling. Say which stage is outstanding rather than only "Checking".
  const pending = firstStalledStage(readiness);
  return pending === null
    ? { level: "warn", text: "Checking…" }
    : { level: "warn", text: "Checking…", reason: `Waiting on ${pending}` };
}
...
function setStatusText(value: string) {
  ...
  fitElementToBox(elements.statusLine);
...
/// The longest prefix of `label` that measures within `maxWidth` in `font`,
/// with an ellipsis when it had to be cut at all.
///
/// Measured rather than counted: a grapheme's width is not a function of how
/// many there are, so a character budget is a guess that is wrong per string
/// and per font.
function fitText(label: string, font: string, maxWidth: number): string {
  try {
    if (measureNaturalWidth(prepareWithSegments(label, font)) <= maxWidth) {
      return label;
    }
    ...
    // still correct, just possibly wide. The CSS ellipsis on these elements is
...
function fitAccountLabel(label: string): string {
  return fitText(label, GH_LABEL_FONT, GH_LABEL_MAX_WIDTH);
}

/// Fit one element's text to the width its own box was given.
///
/// The box is read from the DOM and the font from the element's computed style,
/// so neither is a constant anyone has to keep in step with the stylesheet -
/// move the element, change its font, put it in a narrower panel, and it re-fits
/// itself instead of needing a new number.
///
/// The element's own box must be imposed by its parent, which in practice means
/// a block inside a `min-width: 0` flex item. An element that sizes to its own
/// content reports the content width as its box, every string then "fits", and
/// this quietly does nothing.
///
/// Elements whose box is not laid out yet measure zero; they are left alone and
/// fitted when their panel is opened.
function fitElementToBox(element: HTMLElement): void {
  const shown = element.textContent ?? "";
  const cached = element.dataset.fullText;
  // A caller that replaced the text outright wins over the cache. Without this
  // the element would be refitted from a string it no longer shows, which is
  // the kind of staleness that only appears once something else changes.
  const cacheStillDescribes =
    cached !== undefined &&
    (shown === cached || (shown.endsWith("…") && cached.startsWith(shown.slice(0, -1))));
  const full = cacheStillDescribes ? cached : shown;
  if (!full) return;

  element.dataset.fullText = full;
  // Keep the tooltip in step with the text, but only where the fitter put it
  // there. An element whose text is reused - a status value rewritten on every
  // poll - would otherwise keep the first string it ever showed, and a caller
  // that set its own title deliberately, like a folder naming its full path,
  // must not have it overwritten.
  if (element.dataset.fitOwnsTitle === "1" || !element.title) {
    element.dataset.fitOwnsTitle = "1";
    element.title = full;
  }

  // Ornaments drawn by the element itself take room the text cannot have. The
  // status rows put their health dot in a `::before`, so measuring the text
  // against the whole box would overflow by exactly the dot.
  const width = element.clientWidth - ornamentWidth(element, "::before") - ornamentWidth(element, "::after");
  if (width <= 0) return;

  const style = getComputedStyle(element);
  const font = `${style.fontStyle} ${style.fontWeight} ${style.fontSize} ${style.fontFamily}`;
  element.textContent = fitText(full, font, width);
}

/// How much horizontal room one of an element's pseudo-elements occupies.
/// Zero when there is no such pseudo-element, which is the usual case.
function ornamentWidth(element: HTMLElement, pseudo: "::before" | "::after"): number {
  const style = getComputedStyle(element, pseudo);
  if (style.content === "none") return 0;
...
/// Refit everything marked `data-fit` that is currently on screen. Cheap enough
/// to run on every render and every tab change, which is what keeps a string
/// correct after the thing around it moved.
