...
export function registerGitHubTools(server: McpServer, readOnlySurface: boolean): void {
  ...
  const repositorySegment = z.string().min(1).max(100).regex(/^[A-Za-z0-9._-]+$/);
  ...
  server.registerTool("github_repo_rebind", {
    title: "Point this folder at a different GitHub repository",
    description:
      "Deliberately retarget the selected folder's binding from one GitHub repository to another, updating the " +
      "desktop binding and the local origin remote together. github_repo_ensure will not do this: it reuses an " +
      "existing binding and answers already_bound, because a silent retarget would publish a folder to a " +
      "repository nobody chose. This is the explicit alternative, so it insists on stating what is being " +
      "replaced: expectedOwner and expectedRepo must match the binding that is actually current, and any " +
      "existing origin must still point at that same repository. If either has moved, nothing is changed and " +
      "the current binding is named in the refusal - read github_status and call again. The target repository " +
      "must already exist; it is read through GitHub and its numeric repository id is stored as the new " +
      "identity anchor. A custom Pages domain recorded for the old repository is dropped, because it is not " +
      "evidence about the new one. Returns the previous and new owner, name, repository id, origin and branch. " +
      "This changes no file and pushes nothing.",
    inputSchema: z.object({
      expectedOwner: repositorySegment,
      expectedRepo: repositorySegment,
      targetOwner: repositorySegment,
      targetRepo: repositorySegment,
