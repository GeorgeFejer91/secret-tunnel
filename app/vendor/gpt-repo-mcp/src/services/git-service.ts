...
/** How Git words a HEAD that points at a branch which has no commit yet. */
const UNBORN_HEAD = /unknown revision|ambiguous argument|does not have any commits/;
...
export class GitService {
  ...
  async status() {
    ...
    return { branch, head_sha: headSha, unborn: headSha === null, clean: files.length === 0, files, counts };
  }
   ...
   *
   * `symbolic-ref` answers on a repository whose branch has no commit yet,
   * where `rev-parse HEAD` cannot; it fails only on a detached HEAD, which is
   * exactly what the previous `rev-parse --abbrev-ref HEAD` reported as
   * "HEAD". A folder that is not a repository at all still fails, through the
   * `git status` call this runs beside.
   */
  private async branchName(): Promise<string> {
    try { return (await this.gitText(["symbolic-ref", "--short", "HEAD"], SMALL_GIT_LIMIT)).trim(); }
    catch { return "HEAD"; }
  }

  /**
   * The current commit, or null in a repository with no commit yet.
   *
   * An unborn branch is a normal starting state - `git init` then a first
   * write - not a failure, so it must not turn a status or review call into an
   * error. Every other Git failure still propagates.
   */
  private async headSha(): Promise<string | null> {
    try { return (await this.gitText(["rev-parse", "HEAD"], SMALL_GIT_LIMIT)).trim(); }
    catch (error) {
      if (error instanceof RepoReaderError && UNBORN_HEAD.test(error.message)) return null;
      throw error;
    }
  }
