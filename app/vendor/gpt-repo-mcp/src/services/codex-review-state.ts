...
export function buildCodexReviewState(input: {
  ...
  // An unborn branch gives the review no commit to bind an attestation to,
  // which is the same "no usable git state" answer as having no review at all.
  if (input.gitReview.head_sha === null) return { status: "unavailable", reason: "missing_git_state" };
