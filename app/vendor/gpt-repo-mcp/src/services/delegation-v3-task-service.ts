...
export class DelegationV3TaskService {
  ...
  private async captureBaseline(runId: string): Promise<CodexBaseline> {
    ...
    // A delegation run is defined relative to a baseline commit, so unlike
    // status and review this one genuinely needs a HEAD.
    if (status.head_sha === null) {
      throw new RepoReaderError("GIT_ERROR", "Start a delegation run in a repository that has at least one commit.");
    }
    return {
