...
export async function runProcessWithTail(input: {
  ...
  // A command run inside an approved folder is not confined to it, so task
  // execution, validation and delegation are unavailable while restricted.
  assertCapabilityAllowedInSmartScope("Executing a command");
