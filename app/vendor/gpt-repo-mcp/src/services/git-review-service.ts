...
export class GitReviewService {
  ...
  async review(input: GitReviewInput): Promise<GitReviewResult> {
    ...
    // A branch with no commit yet has no HEAD to pin an expected-HEAD
    // precondition to, so the payloads below that carry one are simply not
    // offered. Reviewing an unborn repository still works; it just cannot
    // suggest a call that would be refused for naming a commit that does not
    // exist.
    ...
    const shipReadiness = await this.readShipReadiness(head, worktreeFingerprint);
    ...
    if (recoverableWorktreePaths.length > 0 && head !== null) {
    ...
    if (stagedPaths.length > 0 && head !== null) {
    ...
    if (head !== null && (stagedRecoveryPaths.length > 0 || recoverRestorePaths.length > 0 || cleanupPaths.length > 0 || discardPaths.length > 0)) {
    ...
    if (stagePaths.length > 0 && happyPathAllowed && head !== null) {
    ...
    if (expectedCommitPaths.length > 0 && happyPathAllowed && head !== null) {
    ...
    if (stagePaths.length === 0 && stagedPaths.length > 0 && !hasStagedExcludedPaths && happyPathAllowed && head !== null) {
  ...
  private async readShipReadiness(headSha: string | null, worktreeFingerprint: string): Promise<NonNullable<GitReviewResult["ship_readiness"]>> {
    try {
