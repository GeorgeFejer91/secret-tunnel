...
import { assertCapabilityAllowedInSmartScope } from "../runtime/smart-scope.js";
...
export async function runGitBounded(input: {
  ...
  // One check for every Git invocation in the server. Git discovers the
  // repository above its working directory, so an approved child folder does
  // not bound it: status, diff, commit and restore would all act on the whole
  // project. Refused at the point of execution, so no route reaches it.
  assertCapabilityAllowedInSmartScope("Git");
