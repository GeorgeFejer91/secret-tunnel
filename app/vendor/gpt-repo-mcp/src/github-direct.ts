...
const USER_AGENT = "SecretTunnel-v3-direct";
...
const SHA = /^[a-f0-9]{40}$/;
/**
 * GitHub's own hard ceiling: regular Git - and the git data API this path uses
 * - refuses a file of 100 MiB or more, and only Git LFS goes above it. Files
 * below it are ordinary, so publishing one is an ordinary publication.
 */
const MAX_FILE_BYTES = 100 * 1024 * 1024;
/**
 * The selection is read and base64-encoded in memory before it is sent, so the
 * set is bounded as well as each file in it.
 */
const MAX_PUBLICATION_BYTES = 100 * 1024 * 1024;
...
async function api(
  ...
  const response = await fetch(`${GITHUB_API}${path}`, {
    ...
    body: payload,
    // Thirty seconds is right for a metadata call and wrong for a large blob:
    // a real upload of tens of megabytes takes longer than that on an ordinary
    // connection, and aborting it would report a working publication as a
    // timeout. A large body gets time proportional to its size, assuming a
    // slow link, with a ceiling so a stalled request still ends.
    signal: AbortSignal.timeout(Math.min(
      30_000 + Math.ceil((payload === undefined ? 0 : Buffer.byteLength(payload)) / 262_144) * 1000,
      600_000
    ))
  });
 ...
 *
 * Same folder the local file tools expose, so the direct path cannot reach
 * outside the folder the user selected. The size limit is deliberately not
 * theirs: those tools read source text into a tool result and are bounded for
 * that, while this one hands bytes straight to GitHub, which accepts anything
 * below 100 MiB. A file is refused here only where GitHub itself would refuse
 * it, and its size is checked before it is read, so an enormous file is
 * declined rather than loaded.
 */
async function readWorkspaceFiles(
  ...
  for (const path of paths) {
    ...
    try {
      ...
      if (!info.isFile()) refuse("file_unreadable", `'${path}' is not a file.`);
      size = info.size;
    } catch (error) {
      if (error instanceof DirectError) throw error;
      refuse("file_unreadable", `'${path}' could not be read from the selected folder.`);
    }
    if (size >= MAX_FILE_BYTES) {
      refuse("file_too_large",
        `'${path}' is ${size} bytes. GitHub refuses any file of ${MAX_FILE_BYTES} bytes or more over regular Git, `
        + "which is what this path uses. Reduce the file or publish it another way; Git LFS is not supported here.");
    }
    total += size;
    if (total > MAX_PUBLICATION_BYTES) {
      refuse("publication_too_large",
        `These files total more than ${MAX_PUBLICATION_BYTES} bytes, which is more than one publication holds in `
        + "memory at once. Publish them in smaller groups.");
    }
    let content: Buffer;
    try { content = await readFile(absolute); }
    catch { refuse("file_unreadable", `'${path}' could not be read from the selected folder.`); }
    // The file can have grown between the two calls.
    if (content.length >= MAX_FILE_BYTES) {
      refuse("file_too_large", `'${path}' grew past ${MAX_FILE_BYTES} bytes while it was being read.`);
    }
...
export function registerDirectGitHubTools(
  ...
  server.registerTool("github_direct_publish", {
    ...
    description:
      ...
      "local repository, the local branch does not advance, nothing is pushed, no desktop repository binding " +
      "is required, and the desktop app performs no part of it. All files land in one " +
      "commit rather than one commit each, so no deployment ever sees a half-published site. There is no force " +
      "update: if the branch moved since its tip was read, this refuses instead of overwriting it. Files are " +
      "sent as bytes, so binaries publish exactly as they are; any file GitHub itself accepts over regular Git, " +
      "meaning anything below 100 MiB, can be published here, and one call reads at most 100 MiB in total. " +
      "Returns the new commit SHA and the branch tip read back from GitHub afterwards.",
    inputSchema: z.object({
