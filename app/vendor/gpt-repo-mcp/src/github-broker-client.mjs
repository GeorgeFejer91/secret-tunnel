...
const ROUTES = new Set(['/github/status', '/github/plan', '/github/apply',
  ...
  '/github/pages_context', '/github/repo_ensure', '/github/repo_rebind', '/github/repo_ship', '/github/pages_ensure']);
const SHA = /^[a-f0-9]{40}$/;
...
export function validateBrokerInput(route, value) {
  ...
  if (route === '/github/repo_rebind') {
    // Every field is required except the branch. A retarget states both the
    // binding it expects to replace and the one it wants, so a request written
    // against stale state cannot be read as a request to move anywhere.
    exactKeys(value, ['expectedOwner', 'expectedRepo', 'targetOwner', 'targetRepo', 'branch']);
    const segment = (field) => {
      const name = value[field];
      requireThat(typeof name === 'string' && /^[A-Za-z0-9._-]{1,100}$/.test(name)
        && name !== '.' && name !== '..' && !name.startsWith('-'),
      'invalid_repository_name', `${field} must be a GitHub owner or repository name.`);
      return name;
    };
    const result = {
      expectedOwner: segment('expectedOwner'), expectedRepo: segment('expectedRepo'),
      targetOwner: segment('targetOwner'), targetRepo: segment('targetRepo')
    };
    if (value.branch !== undefined) {
      requireThat(typeof value.branch === 'string' && value.branch.length > 0 && value.branch.length <= 200
        && !/[\s~^:?*\[\\\x00-\x1f\x7f]/.test(value.branch), 'invalid_branch', 'A branch name must be a plain Git branch name.');
      result.branch = value.branch;
    }
    requireThat(`${result.expectedOwner}/${result.expectedRepo}`.toLowerCase()
      !== `${result.targetOwner}/${result.targetRepo}`.toLowerCase(),
    'invalid_input', 'The target repository is the one already bound; there is nothing to retarget.');
    return result;
  }
...
function projectResponse(route, value) {
  ...
  if (route === '/github/repo_rebind') {
    requireThat(object(value), 'broker_schema', 'Broker returned an invalid rebind result.');
    requireThat(Number.isSafeInteger(value.repositoryId) && value.repositoryId > 0, 'broker_schema', 'Broker returned an invalid repository id.');
    requireThat(value.previousRepositoryId === null || (Number.isSafeInteger(value.previousRepositoryId) && value.previousRepositoryId > 0),
      'broker_schema', 'Broker returned an invalid previous repository id.');
    const remote = (url) => {
      requireThat(/^https:\/\/github\.com\/[A-Za-z0-9._-]{1,100}\/[A-Za-z0-9._-]{1,100}(\.git)?$/.test(url) && !url.includes('@'),
        'broker_schema', 'Broker returned an invalid remote URL.');
      return url;
    };
    return {
      previousOwner: text(value.previousOwner, 100), previousRepo: text(value.previousRepo, 100),
      previousRepositoryId: value.previousRepositoryId ?? null,
      previousOrigin: value.previousOrigin === null || value.previousOrigin === undefined
        ? null : remote(text(value.previousOrigin, 2048)),
      previousBranch: text(value.previousBranch, 512),
      owner: text(value.owner, 100), repo: text(value.repo, 100), repositoryId: value.repositoryId,
      origin: remote(text(value.origin, 2048)), branch: text(value.branch, 512)
    };
  }
...
export function createGitHubHandlers(client, readOnlySurface = false) {
  ...
  return {
    ...
    repoRebind: (args, extra) => invoke('/github/repo_rebind', args, extra),
...
export async function runtimeStatus(client, readOnlySurface, configurationState = 'unconfigured') {
  ...
  if (!readOnlySurface) names.push('github_plan', 'github_apply', 'github_create_repository', 'github_create_repository_apply',
    'github_pages_ensure', 'github_repo_ensure', 'github_repo_rebind', 'repo_ship',
  ...
  // Callers orient from this result, not from the server instructions some
  // hosts drop. Naming the two chains here is what stops a caller inventing a
  // missing capability when it cannot see how publishing is meant to work.
  const publishingPath = readOnlySurface
    ? ['This surface is read-only. No tool here writes to GitHub.']
    : [
      'Edit first: repo_write_file or repo_write_changes.',
      'Managed path, the desktop acts: github_status -> github_repo_ensure -> repo_ship -> github_pages_ensure -> github_pages_status.',
      'Direct path, this server calls api.github.com: github_direct_status -> github_direct_repo_ensure -> github_direct_publish -> github_direct_pages_ensure -> github_direct_pages_status.',
      'Every name in githubTools is registered and callable. There is no per-action toggle to enable. Call the tool and quote any refusal verbatim rather than reporting it as missing.'
    ];
  const result = { ...identity, brokerState, accessMode: readOnlySurface ? 'read' : 'read_write',
    githubTools: names.sort(), publishingPath,
    toolCatalogDigest: createHash('sha256').update(names.sort().join('\n')).digest('hex') };
