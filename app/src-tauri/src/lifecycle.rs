...
/// Releases the in-flight generation if the worker ends without publishing.
///
/// `publish` is the only thing that clears `in_flight_generation`, and every
/// other part of the system reads that field to decide whether work is already
/// under way - including the supervisor that repairs a lost connection, which
/// skips its whole health check while a generation is in flight. So a worker
/// that ends without publishing does not merely lose one start: it wedges the
/// lifecycle in `Starting` permanently and leaves nothing able to retry, which
/// is indistinguishable from the app having quietly given up.
///
/// A panic is the way that happens. This guard runs during unwinding, hands
/// the generation back and records the failure, so the next supervisor tick
/// sees an idle coordinator and starts again.
struct GenerationGuard<'a> {
    state: &'a Arc<Mutex<CoordinatorState>>,
    attempt: &'a Arc<Attempt>,
    hook: &'a Option<Arc<dyn PublishHook>>,
    published: bool,
}

impl GenerationGuard<'_> {
    /// Called on every path that publishes normally, so the drop below only
    /// acts when something ended the worker without one.
...
impl Drop for GenerationGuard<'_> {
    fn drop(&mut self) {
        ...
        // Stopped with a failure, exactly as a start that failed and cleaned
        // up reports itself: the services are not running, the reason is
        // recorded, and the coordinator is idle so the next attempt may begin.
        publish(
            self.state,
            self.attempt,
            ...
            None,
            Some((
                "A start attempt ended without reporting an outcome. The next attempt will start again."
                    .to_string(),
                "worker_ended_unexpectedly".to_string(),
            )),
            self.hook,
        );
    }
...
fn run_worker(
    ...
    let mut generation_guard = GenerationGuard {
        state: &state,
        attempt: &attempt,
        hook,
        published: false,
    ...
    // Every arm below publishes, so the guard has done its job.
    generation_guard.published();
    match outcome {
...
mod tests {
    ...
    /// An endpoint that dies the way a bug dies: mid-start, without returning.
    struct PanickingEndpoint;

    impl LifecycleEndpoint for PanickingEndpoint {
        fn start_services(&self, _attempt: &Attempt, _settings: &Settings) -> Result<(), AppError> {
            panic!("a start attempt panicked");
        }

        fn stop_services(&self, _attempt: &Attempt) -> Result<bool, AppError> {
            Ok(true)
        }
    }

    /// A worker that ends without publishing used to keep `in_flight_generation`
    /// set forever. Everything that decides whether to act reads that field -
    /// including the supervisor that repairs a lost connection, which skips its
    /// entire health check while a generation is in flight. So one panic did
    /// not cost one start; it wedged the app in `Starting` with nothing left
    /// able to retry, which to the user is the app silently giving up.
    #[test]
    fn a_panicking_worker_hands_the_generation_back() {
        let coordinator = Coordinator::new(Arc::new(PanickingEndpoint));
        let settings = sample_settings("C:/fixture/workspace");

        // The panic unwinds the worker thread; the coordinator must not care.
        let generation = coordinator.submit(LifecycleRequest::Start {
            settings: settings.clone(),
            revision: 1,
            retry: false,
        });
        assert_ne!(generation, 0, "a first start must spawn a worker");
        coordinator.wait_for_generation(generation, Duration::from_secs(5));

        assert_eq!(
            coordinator.in_flight_generation(),
            None,
            "a worker that ended without publishing must still release the generation"
        );
        assert!(
            !coordinator.effective_state().is_running(),
            "nothing started, so nothing may claim to be running"
        );
        assert!(
            coordinator.desired_running(),
            "the user still wants it running, which is what lets a retry happen"
        );

        // The real proof: another attempt is admitted afterwards. Before this,
        // every later request coalesced away against the stuck generation.
        let again = coordinator.submit(LifecycleRequest::Start {
            settings,
            revision: 2,
            retry: true,
        });
        assert_ne!(again, 0, "the app must be able to try again after a panic");
        coordinator.wait_for_generation(again, Duration::from_secs(5));
    }
