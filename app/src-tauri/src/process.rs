...
/// v3 on 8789. It is not a preference. Two generations sharing a port cannot
/// both run - the second one finds the port held, its MCP server never starts,
/// and its share has nothing to serve - and the installations are meant to run
/// side by side, so an older one keeps working while a newer one is tried.
const DEFAULT_LOCAL_PORT: &str = "8789";
