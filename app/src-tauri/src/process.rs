...
/// The local MCP port, one per product line: v1 answered on 8787, v2 on 8788,
/// v3 on 8789, v4 on 8790. It is not a preference. Two generations sharing a
/// port cannot both run - the second one finds the port held, its MCP server
/// never starts, and its share has nothing to serve - and the installations
/// are meant to run side by side, so an older one keeps working while a newer
/// one is tried. Moving a generation forward means moving this too; the
/// identifier and the product name are not the whole of it.
const DEFAULT_LOCAL_PORT: &str = "8790";
const MAX_LOG_LINES: usize = 160;
const AUTO_START_RETRY_AFTER: Duration = Duration::from_secs(20);
/// How far the supervisor is allowed to slow down between repeated repairs,
/// as a multiple of `AUTO_START_RETRY_AFTER`. It never stops retrying: a
/// long outage should cost patience, not the connection.
const MAX_REPAIR_BACKOFF: u32 = 9;
const SUBPROCESS_DEADLINE: Duration = Duration::from_secs(15);
const CLEANUP_CONFIRM_DEADLINE: Duration = Duration::from_secs(5);
/// How long the zrok share must stay up before it counts as started. It exits
/// within a second or two when its reserved name is still held, so this only
/// has to outlast that.
const ZROK_START_SETTLE: Duration = Duration::from_secs(3);
/// Bounded on purpose. If three attempts spread over a few seconds cannot get
/// the share up, the cause is not a race and retrying forever would only hide
/// it behind a permanently "starting" window.
const ZROK_START_ATTEMPTS: usize = 3;
const ZROK_START_BACKOFF: Duration = Duration::from_secs(2);
...
impl LifecycleEndpoint for ServiceEndpoint {
    fn start_services(&self, attempt: &Attempt, settings: &Settings) -> Result<(), AppError> {
        ...
        let mcp_pid = mcp.id();
        let zrok = match self.spawn_zrok_with_retry(attempt, settings) {
            Ok(child) => child,
            Err(error) => {
                stop_child(mcp);
                if let Ok(mut runtime) = self.runtime.lock() {
...
impl ServiceEndpoint {
    fn spawn_mcp(&self, attempt: &Attempt, settings: &Settings) -> Result<Child, AppError> {
        ...
        // Started in the background on every launch and every repair, so it must
    ...
    /// Start the share and confirm it is still alive a moment later.
    ///
    /// Spawning only proves the executable started. zrok exits within a second
    /// or two when the reserved name is still held by a share the service has
    /// not released yet, which happens after the app was force-killed and its
    /// own cleanup never ran. Before this check the dead child was stored as
    /// the running share, so the app reported itself healthy while the public
    /// URL answered 502 until someone restarted it by hand.
    ///
    /// The retry re-runs the stale-share cleanup, which is the thing that
    /// actually clears the conflict, then tries again. Bounded: three attempts
    /// with a short backoff, then the real failure is returned.
    fn spawn_zrok_with_retry(
        &self,
        attempt: &Attempt,
        settings: &Settings,
    ) -> Result<Child, AppError> {
        let mut last = None;
        for try_index in 0..ZROK_START_ATTEMPTS {
            if attempt.is_cancelled() {
                return Err(AppError::new("cancelled", "Superseded while starting."));
            }
            if try_index > 0 {
                // The conflicting share is released asynchronously by the zrok
                // service, so clear again and give it a moment.
                clear_stale_zrok_shares_for(
                    settings.zrok_name.clone(),
                    &self.runtime,
                    &self.paths,
                ...
                thread::sleep(ZROK_START_BACKOFF * try_index as u32);
            }
            let mut child = self.spawn_zrok(attempt, settings)?;
            let pid = child.id();
            match child_survived_startup(&mut child, ZROK_START_SETTLE) {
                true => return Ok(child),
                false => {
                    // The child is gone, so forget it. Leaving the pid tracked
                    // would let a later takeover believe a dead share is ours.
                    stop_child(child);
                    if let Ok(mut runtime) = self.runtime.lock() {
                        ...
                        runtime.push_log(
                            "zrok",
                            format!(
                                "Share exited during startup; retrying ({}/{ZROK_START_ATTEMPTS}).",
                                try_index + 1
                            ),
                        );
                    }
                    last = Some(AppError::new(
                        "zrok_share_failed",
                        "The zrok share stopped immediately after starting.",
                    ));
                }
            }
        }
        Err(last.unwrap_or_else(|| {
            AppError::new("zrok_share_failed", "The zrok share could not be started.")
        }))
    }
...
/// 1, 1, 2, 4, 8, ... capped at `MAX_REPAIR_BACKOFF`.
///
/// The first retry stays prompt, because most failures are a single bad
/// share; a genuine outage then costs progressively longer waits instead of
/// a service restart every twenty seconds for hours.
fn repair_backoff(repairs: u32) -> u32 {
    match repairs {
        0 | 1 => 1,
        n => 1u32
            .checked_shl(n - 1)
            .unwrap_or(MAX_REPAIR_BACKOFF)
            .min(MAX_REPAIR_BACKOFF),
    }
}
...
fn clear_stale_zrok_shares_for(
    ...
    for token in tokens {
        ...
        let Ok(mut command) = zrok_command() else {
            return;
        };
        command
            .arg("delete")
...
/// Record the share this attempt just created as ours.
///
/// This is what stops the app deleting its own tunnel. `clear_stale_zrok_shares_for`
/// removes every share registered under our name that we do not know we own,
/// which is right for debris left by a killed process and catastrophic for a
/// share that is serving right now. The share only becomes visible once zrok
/// has registered it, and when zrok is slow that is tens of seconds after the
/// child started, so the wait has to outlast a slow registration rather than a
/// typical one. It returns the moment the share appears, so a healthy start
/// pays a second or two.
...
impl AppState {
    ...
    pub fn snapshot(&self, autostart_enabled: bool) -> Result<StatusDto, AppError> {
        ...
        let exposed = match crate::settings::effective_roots(&settings) {
            Ok(roots) => (
                roots
                    .into_iter()
                    .map(|entry| ExposedRootDto {
        ...
        Ok(StatusDto {
            ...
            exposed_roots: exposed.0,
            roots_error: exposed.1,
    ...
    /// Long-lived backend supervisor: keeps the connection ChatGPT uses alive
    /// for as long as the user intent is still "running".
    ///
    /// It judges health by whether the child processes are actually alive, not
    /// by what the lifecycle remembers about a start that once succeeded. That
    /// distinction is the whole point. A zrok share survives its startup
    /// check, is committed as the running tunnel, and then exits half a minute
    /// later - and the lifecycle still says Running, while ChatGPT cannot
    /// reach anything. The app used to sit in that state until somebody
    /// restarted it by hand.
    ///
    /// A dead child is the only signal acted on, deliberately. Rebuilding on
    /// "the public URL is not answering yet" was tried and is worse than the
    /// disease: a share zrok has not finished registering looks exactly like a
    /// broken one, so the rebuild tore down tunnels a few seconds before they
    /// came up, and the next start deleted the share they had just created. A
    /// process that has exited cannot be a false positive.
    ///
    /// Observed via the coordinator, so it honors `desired_running = false`
    /// after an explicit stop and never runs while a worker is in flight.
    pub fn start_supervisor(&self) {
        let state = self.clone();
        thread::spawn(move || {
            // Consecutive repairs that have not yet produced a working
            // connection. It only slows the loop down; nothing here ever gives
            // up, because an outage at zrok or on the network is precisely the
            // case the app has to keep working through.
            let mut repairs: u32 = 0;
            loop {
                thread::sleep(AUTO_START_RETRY_AFTER * repair_backoff(repairs));
                ...
                if state.children_alive() {
                    repairs = 0;
                    continue;
                }
                state.push_app_log("Services are not running; starting them again.");
                ...
                match state.request_repair(settings) {
                    Ok(()) => state.push_app_log("Connection re-established."),
                    Err(error) => {
                        state.push_app_log(format!("Reconnect attempt failed: {}", error.message))
                    }
                }
            }
        });
    ...
    /// Whether both committed children are still alive, forgetting any that
    /// has exited.
    ///
    /// Deliberately not `is_running`, which asks the lifecycle first: the
    /// whole point of the supervisor is the case where the lifecycle still
    /// believes a start that succeeded minutes ago is still true.
    fn children_alive(&self) -> bool {
        match self.runtime.lock() {
            Ok(mut runtime) => {
                runtime.cleanup_exited();
                runtime.is_running()
    ...
    /// Rebuild the services under the configuration already in force.
    ///
    /// Deliberately a Reconfigure rather than a Start: an equivalent Start is
    /// coalesced away while the lifecycle still believes it is running, which
    /// is exactly the state a repair is requested from.
    fn request_repair(&self, settings: Settings) -> Result<(), AppError> {
        let revision = settings_revision(&settings);
        let generation = self
            .coordinator
            .submit(LifecycleRequest::Reconfigure { settings, revision });
        self.await_lifecycle(generation, LifecycleState::Running, "reconnect")
    }
...
/// One approved Smart folders match, as the window shows it.
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct SmartScopeMatchDto {
    pub project: String,
    pub folder: String,
}

/// The stored scope. Distinct from `exposed_roots` below on purpose: this is
/// what was configured, that is what is being served right now, and a person
/// looking at a restricted endpoint needs to be able to tell the two apart.
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct SmartScopeDto {
    pub name: String,
    pub matches: Vec<SmartScopeMatchDto>,
}

/// One root the MCP config currently exposes.
#[derive(Debug, Clone, Serialize)]
#[serde(rename_all = "camelCase")]
pub struct ExposedRootDto {
    pub repo_id: String,
...
pub struct SettingsDto {
    ...
    pub smart_scope: Option<SmartScopeDto>,
}
...
impl SettingsDto {
    fn from_settings(settings: &Settings, paths: &AppPaths) -> Self {
        Self {
            ...
            extra_folders: settings.extra_folders.clone(),
            smart_scope: settings.smart_scope.as_ref().map(|scope| SmartScopeDto {
                name: scope.name.clone(),
                matches: scope
                    .matches
                    .iter()
                    .map(|entry| SmartScopeMatchDto {
...
pub struct StatusDto {
    ...
    /// Whether the desktop GitHub feature is switched on. Carried here so the
    /// window can show it on the existing status poll rather than shelling out
    /// to git every few seconds for a single boolean.
    ...
    /// The roots the managed MCP config exposes as things stand, resolved by
    /// the same function that writes that config. Empty with a reason below
    /// rather than guessed at when the scope cannot be honoured.
    pub exposed_roots: Vec<ExposedRootDto>,
    /// Why nothing is exposed, when that is the case.
...
/// True when the child is still running at the end of the settle window.
///
/// Returns as soon as the child is seen to have exited, so a failure costs
/// almost nothing; only the success path pays the full window.
fn child_survived_startup(child: &mut Child, settle: Duration) -> bool {
...
fn validate_external_url(url: &str) -> Result<String, AppError> {
    ...
    // `&` is deliberately absent: it is ordinary query-string syntax, and a
    // prefilled GitHub form is nothing but query string. It was only ever
    // dangerous because the opener below ran through `cmd`, which is no longer
    // true. The rest stay rejected as cheap defence against a link that is not
    // really a web address.
...
/// Open a link in the user's default browser without a shell.
///
/// `cmd /C start` was the obvious way and the wrong one: `cmd` re-parses its
/// arguments with its own rules, so every metacharacter had to be banned, and
/// `&` is exactly what a prefilled form URL is made of. `FileProtocolHandler`
/// takes the URL as one argument and hands it straight to the default
/// handler, so nothing re-interprets it.
...
fn configure_mcp_environment(command: &mut Command, paths: &AppPaths, settings: &Settings) {
    command
        ...
        // The System Prompt tab, made machine-facing. The bundled server
        // appends this to the `instructions` it returns from `initialize`, so
        // it is what the model reads before it touches a shared file - not
        // setup text for the person. Always set, never conditionally, so a
        // cleared prompt cannot leave the previous one behind in the child's
        // environment.
        .env(
            "GPT_REPO_USER_INSTRUCTIONS",
            crate::system_prompt::effective_prompt(&settings.system_prompt),
        )
        .stdout(Stdio::piped())
        .stderr(Stdio::piped());

    // Smart folders narrows the roots, which the config already carries. This
    // flag carries the other half: Git resolves the repository *above* whatever
    // directory it runs in, so pointing it at an approved subfolder would still
    // reach the whole project. The server refuses those capabilities outright
    // rather than appearing to confine them. Always set or removed, never left
    // over from a previous configuration.
    if settings.smart_scope.is_some() {
        command.env("GPT_REPO_SMART_SCOPE", "1");
    } else {
        command.env_remove("GPT_REPO_SMART_SCOPE");
    }

    // Hand the GitHub broker's address and token to the MCP child through its
    // environment. Deliberately not through a file: the only writable location
    // the child shares with us is the workspace, which is exposed over the
    // tunnel, so a token written there would be readable by the very clients it
    // is meant to authenticate. When the broker did not start these are absent
    // and the child registers no GitHub tools at all.
    for key in ["SECRET_TUNNEL_BROKER_URL", "SECRET_TUNNEL_BROKER_TOKEN"] {
...
/// Where the bundled server and its companion live.
///
/// The path is normalized before it is returned, because a resource directory
/// can arrive canonicalized and Windows canonical paths carry the
/// extended-length `\\?\` prefix. Node's ESM loader cannot resolve a script
/// given to it that way: the process starts and dies before the module runs,
/// with nothing on stdout or stderr to say why. The MCP server escaped that
/// only because its own caller normalized the directory again before building
/// the script path, which left every other caller to rediscover the same trap.
/// Normalizing once, here, is what makes the path usable for all of them.
pub(crate) fn bundled_mcp_runtime_dir() -> Option<PathBuf> {
    for root in resource_search_dirs() {
        for candidate in [
            ...
            {
                return Some(PathBuf::from(normalize_windows_verbatim_prefix(
...
/// zrok's state directory for *this* application.
///
/// zrok defaults to one shared folder per user, `%USERPROFILE%\.zrok2`. Every
/// zrok-based app on the machine then reads and writes the same environment
/// record, the same identity and the same share list - so one app's routine
/// cleanup deletes another app's live share, and one app disabling its
/// environment takes the other one down with it. That is not hypothetical: it
/// is how a Secret Tunnel v1 install and a v2 install managed to break each
/// other's tunnels while each behaved correctly on its own.
///
/// So this app keeps zrok's state in a folder it owns, beside its own
/// settings, and passes it to every zrok invocation through ZROK_HOME. The
/// account behind it is still the user's one zrok account, which is right -
/// what is no longer shared is the on-disk state.
///
/// The first call seeds the folder from the user-global one when that exists,
/// by copying, never moving: an installation that was already working keeps
/// working without a new zrok enrolment, and any other app's copy is left
/// exactly as it was.
fn zrok_home() -> Option<PathBuf> {
    static HOME: OnceLock<Option<PathBuf>> = OnceLock::new();
    HOME.get_or_init(prepare_zrok_home).clone()
}

/// Resolve and, on the first call of the process, populate that folder.
fn prepare_zrok_home() -> Option<PathBuf> {
    // No settable configuration directory means no folder this app can call
    // its own; fall back to zrok's default rather than inventing a location.
    let home = crate::settings::app_paths()
        .ok()?
        .config_dir
        .join("zrok-home");
    let enrolled = home.join("environment.json").is_file();
    if !enrolled {
        std::fs::create_dir_all(&home).ok()?;
    }
    // A note for whoever finds this folder later and wonders which app owns it.
    // Written on every start rather than only when the folder is created: a
    // folder carried over from the previous generation arrives with that
    // generation's note inside it, and a note naming the wrong owner is worse
    // than no note at all.
    let _ = std::fs::write(
        home.join("OWNER.txt"),
        concat!(
            "zrok state owned by Secret Tunnel v4.\n",
            "Passed to every bundled zrok2 call as ZROK_HOME, so this installation\n",
            "never shares zrok's environment, identity or share list with another app.\n",
            "Delete it only while Secret Tunnel v4 is closed; zrok is then re-enrolled.\n"
        ),
    );
    if enrolled {
        return Some(home);
    }
    if let Some(legacy) = legacy_zrok_home() {
        copy_zrok_home(&legacy, &home);
    }
    Some(home)
}

/// zrok's own default location, which is what an existing enrolment lives in.
fn legacy_zrok_home() -> Option<PathBuf> {
    let profile = std::env::var_os("USERPROFILE").or_else(|| std::env::var_os("HOME"))?;
    let legacy = PathBuf::from(profile).join(".zrok2");
    legacy.join("environment.json").is_file().then_some(legacy)
}

/// Copy an enrolment across. Small and flat by design: environment.json,
/// metadata.json and an `identities` directory. Anything unreadable is skipped
/// rather than failing the app's start; a missing enrolment surfaces later as
/// the ordinary "zrok needs enable" state.
fn copy_zrok_home(from: &Path, to: &Path) {
        ...
        match entry.file_type() {
            Ok(kind) if kind.is_dir() => {
                if std::fs::create_dir_all(&target).is_ok() {
                    copy_zrok_home(&source, &target);
                }
            }
            // The agent socket is a live endpoint, not state worth copying.
            Ok(kind) if kind.is_file() && name != "agent.socket" => {
...
/// A zrok invocation pinned to this application's own zrok state.
///
/// Every zrok call goes through here. A call that forgot it would silently act
/// on the shared user-global folder again, which is the whole class of bug
/// this exists to end.
///
/// The console window is suppressed here for the same reason: zrok is called
/// unattended - on status polls, and repeatedly while a stale share is being
/// cleared - and a call that forgot it would flash a black window over whatever
/// the user was doing. Building it in means a new call site cannot reintroduce
/// that, and the individual call sites no longer have to remember.
fn zrok_command() -> Result<Command, AppError> {
    let mut command = Command::new(bundled_zrok_command()?);
    if let Some(home) = zrok_home() {
        command.env("ZROK_HOME", home);
    }
    suppress_console_window(&mut command);
    Ok(command)
}
...
pub(crate) fn bundled_executable(command: &str) -> Option<PathBuf> {
...
mod tests {
    use super::{
        ...
        copy_zrok_home, repair_backoff, stale_zrok_share_tokens_from_json,
        status_probe_path_from_environment, validate_zrok_token,
    ...
    /// A script path is only useful if node can load it, and on Windows a
    /// canonical path cannot be: the `\\?\` prefix defeats the ESM loader, the
    /// process exits before the module body runs, and stdout and stderr are
    /// closed, so the failure looks exactly like a missing bundle. The Network
    /// companion shipped that way in v3 and v4 - it spawned every ten seconds
    /// and died every time, and the panel could only say the service was
    /// unavailable.
    ///
    /// The resource directory is fed in with the prefix on it, because that is
    /// what the packaged app gets and what a test run does not: reading
    /// whatever this machine happens to resolve would pass without proving
    /// anything.
    #[cfg(windows)]
    #[test]
    fn the_bundled_runtime_path_is_one_node_can_load() {
        let _guard = ENV_LOCK.lock().unwrap_or_else(|error| error.into_inner());
        let Ok(crate_dir) = env::current_dir() else {
            return;
        };
        // Only a prepared checkout has a runtime to find; without one there is
        // no path to inspect and nothing this can assert.
        if !crate_dir
            .join("resources")
            .join("gpt-repo-mcp")
            .join("dist")
        ...
        env::set_var(
            "SECRET_TUNNEL_RESOURCE_DIR",
            format!(r"\\?\{}", crate_dir.display()),
        );
        let found = bundled_mcp_runtime_dir();
        match previous {
            Some(value) => env::set_var("SECRET_TUNNEL_RESOURCE_DIR", value),
            None => env::remove_var("SECRET_TUNNEL_RESOURCE_DIR"),
        }

        let dir = found.expect("a prepared runtime must still be found through a verbatim path");
        let shown = dir.to_string_lossy().into_owned();
        assert!(
            !shown.starts_with(r"\\?\"),
            "the runtime directory must not keep the extended-length prefix, \
             because every script path is built from it: {shown}",
        );
    }

    /// The default port belongs to the generation, not to the code that last
    /// touched this file. v1 took 8787 and every generation since has taken
    /// the next one, because two of them sharing a port means the newer
    /// install finds it held and never serves anything.
    ///
    /// Turning the generation over moves the product name, the identifier and
    /// the version; this constant sits somewhere else entirely and was missed
    /// on the v4 turn. Deriving the expected port from the crate version is
    /// what makes the next turn fail here instead of on someone's machine.
    #[test]
    fn the_default_port_follows_the_generation() {
        use super::DEFAULT_LOCAL_PORT;

        let generation: u16 = env!("CARGO_PKG_VERSION")
            .split('.')
            .nth(1)
            .and_then(|minor| minor.parse().ok())
            .expect("the crate version's minor field names the generation");
        assert_eq!(
            DEFAULT_LOCAL_PORT.parse::<u16>().unwrap(),
            8786 + generation,
            "generation {generation} must answer on {}, not {DEFAULT_LOCAL_PORT} - \
             see DEFAULT_LOCAL_PORT",
            8786 + generation,
        );
    }

    /// Seeding this application's own zrok folder from the shared user-global
    /// one is what lets an installation that already worked keep working
    /// without a new enrolment. It must bring the enrolment across intact,
    /// leave the original untouched, and not copy the agent socket, which is a
    /// live endpoint rather than state.
    #[test]
    fn seeding_the_zrok_home_copies_the_enrolment_and_nothing_live() {
        let root = env::temp_dir().join(format!("secret-tunnel-zrok-home-{}", std::process::id()));
        let from = root.join("legacy");
        let to = root.join("owned");
        let _ = fs::remove_dir_all(&root);
        fs::create_dir_all(from.join("identities")).unwrap();
        fs::create_dir_all(&to).unwrap();
        fs::write(from.join("environment.json"), b"enrolment-bytes").unwrap();
        fs::write(from.join("metadata.json"), b"version-bytes").unwrap();
        fs::write(from.join("agent.socket"), b"").unwrap();
        fs::write(
            from.join("identities").join("environment.json"),
            b"identity",
        )
        .unwrap();

        copy_zrok_home(&from, &to);

        assert_eq!(
            fs::read(to.join("environment.json")).unwrap(),
            b"enrolment-bytes",
            "the enrolment must arrive byte for byte"
        );
        assert_eq!(
            fs::read(to.join("metadata.json")).unwrap(),
            b"version-bytes"
        );
        assert_eq!(
            fs::read(to.join("identities").join("environment.json")).unwrap(),
            b"identity",
            "the identity directory must come across too"
        );
        assert!(
            !to.join("agent.socket").exists(),
            "a live agent socket is not state to copy"
        );
        assert!(
            from.join("environment.json").is_file(),
            "the shared folder another app may still use must be left alone"
        );
        let _ = fs::remove_dir_all(&root);
    }

    /// The supervisor waits longer after each failed repair, but never stops.
    /// A backoff that saturated to zero, or that grew without a ceiling, would
    /// either hammer zrok during an outage or abandon the connection for
    /// hours - both of which look to the user like the app giving up.
    #[test]
    fn repair_backoff_grows_and_then_stops_growing() {
        assert_eq!(repair_backoff(0), 1, "the first check must not be delayed");
        assert_eq!(
            repair_backoff(1),
            1,
            "one bad share deserves a prompt retry"
        );
        assert_eq!(repair_backoff(2), 2);
        assert_eq!(repair_backoff(3), 4);
        assert_eq!(repair_backoff(4), 8);
        for repairs in 5..1000u32 {
            let steps = repair_backoff(repairs);
            assert!(
                (1..=super::MAX_REPAIR_BACKOFF).contains(&steps),
                "backoff after {repairs} repairs must stay bounded and nonzero, got {steps}"
            );
        }
        assert_eq!(repair_backoff(u32::MAX), super::MAX_REPAIR_BACKOFF);
    }

    #[cfg(windows)]
    #[test]
    fn finds_listening_pid_on_local_port() {
        use super::{listening_pids_on_local_port, local_port};

        // Built from the port in force rather than a literal: the port moves
        // with the product line, and a fixture pinned to an older generation's
        // port tests nothing about this one.
        let port = local_port();
        let output = format!(
            "
  TCP    127.0.0.1:{port}         0.0.0.0:0              LISTENING       44648
               TCP    127.0.0.1:1420         0.0.0.0:0              LISTENING       51980
               TCP    [::1]:{port}             [::]:0                 LISTENING       44648
"
        );
...
mod open_url_tests {
    ...
    /// The Connect GitHub button opened nothing because this validator treated
    /// `&` as a shell metacharacter, which it only was while the opener went
    /// through `cmd`. A prefilled form URL is mostly ampersands.
    #[test]
    fn a_prefilled_registration_url_is_openable() {
        let url = crate::github::account::REGISTRATION_URL;
        assert!(
            url.contains('&'),
            "the fixture must exercise the query string"
        );
        assert_eq!(validate_external_url(url).unwrap(), url);
    }

    #[test]
    fn links_that_are_not_web_addresses_are_still_refused() {
        for bad in [
            "file:///c:/windows",
            "javascript:alert(1)",
...
#[cfg(test)]
mod zrok_startup_tests {
    use super::child_survived_startup;
    use std::process::{Command, Stdio};
    use std::time::{Duration, Instant};

    /// The defect this exists for: after a force-kill, the next launch spawned
    /// zrok, zrok exited at once because its reserved name was still held, and
    /// the dead child was stored as the running share. The app then reported
    /// itself healthy while the public URL answered 502 until a manual
    /// restart. Spawning is not starting.
    #[test]
    fn a_child_that_exits_immediately_is_not_a_started_share() {
        let Ok(mut child) = exiting_child() else {
            return;
        };
        let started = Instant::now();
        assert!(
            !child_survived_startup(&mut child, Duration::from_secs(3)),
            "a process that exited must never count as a running share"
        );
        // And the failure is noticed promptly rather than after the full window.
        assert!(started.elapsed() < Duration::from_secs(3));
        let _ = child.wait();
    }

    #[test]
    fn a_child_that_keeps_running_counts_as_started() {
        let Ok(mut child) = sleeping_child() else {
            return;
        };
        assert!(child_survived_startup(
            &mut child,
    ...
    #[cfg(windows)]
    fn sleeping_child() -> std::io::Result<std::process::Child> {
        Command::new("cmd")
