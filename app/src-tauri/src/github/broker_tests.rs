...
/// Under a smart scope every GitHub route refuses, including the one that hands
/// the direct path a credential.
///
/// The tools are also left out of the server's catalogue, but that is
/// presentation: this is the refusal a client holding an older tool list runs
/// into. The stored binding is untouched - restoring full access restores it.
#[test]
fn every_github_route_refuses_under_a_smart_scope() {
    let profile = Profile::new();
    let coordinator = coordinator(&profile);
    let paths = AppPaths {
        config_dir: profile.0.clone(),
        settings_path: profile.0.join("settings.json"),
        managed_config_path: profile.0.join("gpt-repo-mcp.config.json"),
    ...
    let project = profile.0.join("project");
    fs::create_dir_all(project.join("For-AI")).unwrap();
    let canonical = |path: &std::path::Path| {
        crate::settings::normalize_windows_verbatim_prefix(
            &fs::canonicalize(path).unwrap().to_string_lossy(),
        )
    };
    let mut settings = crate::settings::load_or_create_settings(&paths).unwrap();
    settings.workspace_path = Some(canonical(&project));
    settings.github.enabled = true;
    settings.github.binding = Some(crate::settings::RepositoryBinding {
    ...
    settings.smart_scope = Some(crate::settings::SmartScope {
        name: "For-AI".to_string(),
        matches: vec![crate::settings::SmartFolderMatch {
            project: canonical(&project),
            folder: canonical(&project.join("For-AI")),
        }],
    });
    crate::settings::save_settings(&paths, &settings).unwrap();
    ...
    for (path, input) in routes {
        let error = route(path, &input, &coordinator).expect_err(&format!("{path} must refuse"));
        assert_eq!(error.code, "smart_scope_restricted", "{path}");
    }

    // The binding is still stored, so restoring full access restores it too.
    let stored = crate::settings::load_or_create_settings(&paths).unwrap();
    assert_eq!(
        stored
            .github
            .binding
            .as_ref()
            .map(|binding| binding.repo.clone()),
        Some("site".to_string())
    );

    // Clearing the scope is the one thing that gives the routes back, and it is
    // not something any of these routes can do.
    let mut cleared = stored;
    cleared.smart_scope = None;
    crate::settings::save_settings(&paths, &cleared).unwrap();
    let restored = route("/github/status", &json!({}), &coordinator);
    assert!(
        restored.is_ok() || restored.unwrap_err().code != "smart_scope_restricted",
        "clearing the scope must give the GitHub routes back"
    );
}
