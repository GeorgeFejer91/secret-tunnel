...
/// Before and after a deliberate retarget, so a caller can see exactly what
/// moved.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct RebindOutcome {
    ...
    pub previous_repository_id: Option<u64>,
...
impl GitHubCoordinator {
    ...
    /// `pub(super)` for the broker, which has to make the same smart-scope
    /// decision this type makes. Reads only; nothing here writes.
    pub(super) fn settings(&self) -> Result<Settings, AppError> {
    ...
    fn ensure_write(&self, settings: &Settings) -> Result<(), AppError> {
        // A commit is repository-wide whichever window asked for it, so the
        // desktop buttons refuse under a smart scope exactly as the broker does.
    ...
    /// Point the already-selected folder at a different GitHub repository.
    ///
    /// `ensure_repository` refuses to retarget and `bind` refuses when origin
    /// already points elsewhere, both deliberately: a silent retarget publishes
    /// a folder to a repository nobody chose. Neither refusal left any way to
    /// change one's mind, which is what this is - the deliberate version, where
    /// the caller states the binding it believes is current and anything else
    /// refuses rather than guessing.
    ///
    /// Everything it touches already exists: the same identity resolution the
    /// binding uses, the same remote writer, the same settings guard.
    pub fn rebind_repository(
        ...
        target_repo: &str,
        branch: Option<String>,
    ) -> Result<RebindOutcome, AppError> {
        let settings = self.settings()?;
        self.ensure_write(&settings)?;
        let root = Self::workspace_root(&settings)
            .ok_or_else(|| AppError::new("no_workspace", "Select a folder first."))?;
        let expected_owner = validate_segment(expected_owner, "owner")?;
        let expected_repo = validate_segment(expected_repo, "repository")?;
        let target_owner = validate_segment(target_owner, "owner")?;
        let target_repo = validate_segment(target_repo, "repository")?;
        let branch = branch.unwrap_or_else(|| "main".to_string());
        super::git::validate_branch_name(&branch)?;
        ...
        let previous = settings
            ...
            .ok_or_else(|| {
                AppError::new(
                    "not_bound",
                    "This folder is not bound to a GitHub repository, so there is nothing to retarget.",
                )
            })?;
        let previous_origin = service
            .remotes(&root, &cancel)?
            .into_iter()
        ...
        // Identity before mutation: an unreachable or nonexistent target is a
        // refusal, and the numeric id it answers with is the anchor stored on
        // the new binding, exactly as `ensure_repository` stores it.
        let repository_id = super::account::resolve_repository_id(
            &self.paths.config_dir,
            &target_owner,
            &target_repo,
        )?;

        let origin = format!("https://github.com/{target_owner}/{target_repo}.git");
        let binding = RepositoryBinding {
            ...
            pages_url: None,
            // Deliberately dropped: a domain GitHub confirmed for the old
            // repository is not evidence about the new one.
            pages_domain: None,
        };
        self.change_settings(|| {
            let mut settings = self.settings()?;
            // Re-read under the guard. Nothing may have moved since the checks
            // above, or this would retarget a state nobody inspected.
            if Self::workspace_fingerprint(&settings) != fingerprint {
                return Err(AppError::new(
                    "workspace_changed",
                    "The selected folder changed while the repository was being retargeted.",
                ));
            }
            let current = settings
                ...
                .binding
                .clone()
                .ok_or_else(|| AppError::new("not_bound", "The binding was removed."))?;
            rebind_precondition(
                &current,
                &expected_owner,
                &expected_repo,
            ...
            let _repo_guard = repository_guard(&service, &root)?;
            // Origin first: a saved binding whose remote still pointed at the
            // old repository would publish to the old repository. The reverse
            // order leaves a remote the existing publication checks refuse.
            service.set_remote(&root, "origin", &origin, &cancel)?;
            settings.github.binding = Some(binding.clone());
            if let Err(error) = save_settings(&self.paths, &settings) {
                return Err(AppError::new(
                    "origin_changed_binding_not_saved",
                    format!(
                        "Origin now points at {target_owner}/{target_repo}, but the desktop binding could not be saved: {}. Nothing was committed or pushed. Inspect the settings before retrying.",
                        error.message
                    ),
                ));
            }
        ...
        Ok(RebindOutcome {
            ...
            previous_repo: previous.repo,
            previous_repository_id: previous.repository_id,
            previous_origin,
            previous_branch: previous.integration_branch,
            owner: target_owner,
            repo: target_repo,
            repository_id,
            origin,
...
/// Whether a bound workspace may be deliberately retargeted.
///
/// Both halves are about acting on state the caller actually saw. The request
/// must name the binding that is current, so a rebind written against a stale
/// reading refuses instead of moving a folder that has since been pointed
/// somewhere else; and origin must be absent or still name that same old
/// repository, so a remote configured by hand outside this app is never
/// silently replaced.
pub(super) fn rebind_precondition(
    binding: &RepositoryBinding,
    expected_owner: &str,
    expected_repo: &str,
    origin: Option<&str>,
) -> Result<(), AppError> {
    if !binding.owner.eq_ignore_ascii_case(expected_owner)
        || !binding.repo.eq_ignore_ascii_case(expected_repo)
    {
        return Err(AppError::new(
            "binding_mismatch",
            format!(
                "This folder is bound to {}/{}, not the {expected_owner}/{expected_repo} this request expected. Read the current binding and retry.",
                binding.owner, binding.repo
            ),
        ));
    }
    if let Some(url) = origin {
        let expected = format!("{expected_owner}/{expected_repo}").to_ascii_lowercase();
        if super::publication::github_repository(url)? != expected {
            return Err(AppError::new(
                "origin_mismatch",
                format!("Existing origin does not point at {expected_owner}/{expected_repo} and was not changed. Review it locally before retargeting."),
            ));
        }
    }
    Ok(())
