...
pub fn app_paths() -> Result<AppPaths, AppError> {
    let config_dir = match config_dir_override()? {
        ...
        None => {
            let dirs =
                ProjectDirs::from("com", "GeorgeFejer", "Secret Tunnel v2").ok_or_else(|| {
...
mod tests {
    use super::{
        ...
        settings_revision, validate_public_path_token, validate_zrok_name, AccessMode, AppPaths,
        ApprovalMode, GitHubSettings, RepositoryBinding, Settings,
    };
    use std::sync::Mutex;
    use std::{env, fs};
