...
pub fn run() {
    ...
    let network_state = std::sync::Arc::new(network::NetworkState::new(paths.clone()));
    // The loopback bridge the MCP server reaches the Storage Box through. It
    // publishes its address and token into the environment the server inherits,
    // so it has to exist before anything is spawned.
    let storage_state = storage::StorageState::new(paths.clone());
    if let Err(error) = storage_state.start_broker() {
        eprintln!("Storage broker unavailable: {}", error.message);
    }
    ...
    let builder = tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            if std::env::var("SECRET_TUNNEL_ACTIONS_ONLY").as_deref() != Ok("1") {
                app.state::<std::sync::Arc<network::NetworkState>>()
                    .start_supervisor();
            }
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::add_extra_folder,
            commands::remove_extra_folder,
            commands::smart_folders_scan,
            commands::smart_folders_apply,
            commands::smart_folders_clear,
        ...
        .on_window_event(|window, event| {
            ...
            {
                window
                    .state::<std::sync::Arc<network::NetworkState>>()
                    .stop();
                window
                    .state::<std::sync::Arc<storage::StorageState>>()
                    .shutdown();
